...
found:
  p->pid = allocpid();
  // program starts with negative value of ticket variable
  p->tickets = -1;
  // ticks 0
  p->pticks = 0;
  // pass is 0
  p->pass = 0;
  // stride also 0 initialized by sched_tickets
  p->stride = 0;
  p->state = USED;
...
void lot_sched()
{
  ...
  for(p = proc; p< &proc[NPROC]; p++)
  {
    ...
    if(p->state == SLEEPING)
    {
      if(p->pid == 3)
      {
        printf("Process with pid %d is sleeping\n", p->pid);
      }
    }
    if(p->state == RUNNABLE)
    {
      ...
      else
      {
          ...
          p->pticks = p->pticks + 1;
          c->proc = p;
          printf("Ran in else program %d now for 1 tick, ticks are now %d\n", p->pid, p->pticks);
          swtch(&c->context, &p->context);
          // Process is done running for now.
          // It should have changed its p->state before coming back.
          c->proc = 0;
