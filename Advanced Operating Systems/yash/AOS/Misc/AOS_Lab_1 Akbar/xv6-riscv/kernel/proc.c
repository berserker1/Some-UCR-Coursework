...
int 
info(int userInput){
  int tmp=cnt_syscalls;
  tmp--;
  printf("hi there! i am info call in proc.c");
  printf("\n");
  ...
  // printf("count of sys call from bootup of OS is %d", cnt_syscalls);
  int cnt_processes = 0;
  switch (userInput)
  {
    case 0:
    ...
    for (p = proc; p < &proc[NPROC]; p++)
    {
      ...
      if (p->state == RUNNABLE || p->state == RUNNING || p->state == ZOMBIE || p->state == SLEEPING) // verify is sleeping is considered as active state in xv6
      {
        cnt_processes++;
      }
    ...
    case 1:
    printf("\nIn switch case 1");
    //  printf("cnt active process %d", tmp);
    //  printf(" \n");
    //the total number of system calls that has made so far since the system boot up.
    /* code */
    return tmp;
    // break;
    ...
    //the number of free memory pages in the system. If there is one free page left,the return value should be 1 (= 1 page).
    /* code */
    return getTotalNumberOfFreePages();
...
• int syscall_count: the total number of system calls that the current process has made so
...
// todo
// printf("p->ppid is %d -- p->syscall_count is %d -- p->page_usage is %d ",(&p->ppid),(p->syscall_count),(p->page_usage));
