...
int nextpid = 1;
int next_tid = 1; // variable to store the number of threads currently allocated.

struct spinlock pid_lock;
struct spinlock tid_lock; // Declaring a mutual exclusion spinlock for threads
...
// Return the thread id to a new thread allocated.
// Working is similar to the allocpid() function
int alloctid() {
  int tid;
...
freeproc(struct proc *p)
{
  ...
  // Unmap page tables for threads, not deallocate it
...
exit(int status)
{
  ...
  // When all the threads are deallocated, assign to init.
  if(p->tid == 0)
    reparent(p);
...
// clone a parent thread similar to fork()
// Shares the address space of the parent.
// Uses the starting address of the stack sent by the caller function to allocate space.
int clone(void* stack) {
  ...
  else
  {
    ...
    // Allocate thread.
    if ((childProcess = allocthread()) == 0)
    {
      return -1;
    }
    else
    {
      // Use the same pagetable as parent
      childProcess->pagetable = p->pagetable;

      // map the trapframe page of the new proceess just below the trampoline page, for
      // trampoline.S.
      if (mappages(childProcess->pagetable, TRAPFRAME - (PGSIZE * childProcess->tid), PGSIZE,
                   (uint64)(childProcess->trapframe), PTE_R | PTE_W) < 0)
      {
